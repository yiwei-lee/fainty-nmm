...
public class Game {
	...
	public Game(int board[], Color color, int phase) {
		...
		if (phase == 1) {
			...
			if (blackUnplacedMen == 0 && whiteUnplacedMen == 0)
				this.phase = 2;
		} else {
			...
			if (blackLeftMen == 3 && whiteLeftMen == 3)
				this.phase = 3;
		}
	}
	...
	public int placeMan(Color color, int x, int y) throws WrongTurnException,
			InvalidPlacementException {
		int toReturn = 0;
		assert (color != null);
		if (removalTurn != null) {
			throw new WrongTurnException(
					"Not a turn to place man, time to remove!");
		}
		if (turn != color) {
			throw new WrongTurnException("Not player " + color
					+ "'s turn to place man!");
		}
		if (phase != 1) {
	...
	public void moveMan(Color color, int x1, int y1, int x2, int y2)
			throws WrongTurnException, InvalidMovementException {
		if (removalTurn != null) {
			throw new WrongTurnException(
					"Not a turn to move man, time to remove!");
		}
		if (turn != color) {
			throw new WrongTurnException("Not player " + color
					+ "'s turn to move man!");
		...
		if ((color == Color.BLACK && blackLeftMen <= 3)
				|| (color == Color.WHITE && whiteLeftMen <= 3)) {
	...
	public void flyMan(Color color, int x1, int y1, int x2, int y2)
		...
		if (removalTurn != null) {
			throw new WrongTurnException(
					"Not a turn to fly man, time to remove!");
		}
		if (turn != color) {
		...
		if ((color == Color.BLACK && blackLeftMen <= 3)
			...
			throw new InvalidMovementException("Player " + color
					+ " can only move man!");
		}
		if (checkNewMill(color, x2, y2)) {
			removalTurn = color;
	...
	public void removeMan(int x, int y) throws WrongTurnException,
			InvalidRemovalException {
		if (removalTurn == null) {
			throw new WrongTurnException("Not the turn to remove!");
		}
		board.removeMan(removalTurn, x, y);
		if (removalTurn == Color.BLACK)
			whiteLeftMen--;
		else
			blackLeftMen--;
		if (phase == 2 && (blackLeftMen == 3 || whiteLeftMen == 3))
			phase = 3;
		if (whiteLeftMen == 2)
			winner = Color.BLACK;
		if (blackLeftMen == 2)
